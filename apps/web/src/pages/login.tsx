			...
			<Card className="w-full max-w-sm">
				...
				<CardContent>
					<form onSubmit={handleSubmit} className="space-y-4">
						...
						{loginMutation.isError && <p className="text-sm text-destructive">{t("pages.login.invalidCredentials")}</p>}
						<Button type="submit" className="w-full" disabled={loginMutation.isPending}>
							<LogIn />
							{loginMutation.isPending ? t("pages.login.submitting") : t("pages.login.submit")}
						</Button>
						<p className="text-center text-xs text-muted-foreground">{t("pages.login.hint")}</p>
					</form>
				</CardContent>
			</Card>
		</div>
