			...
			<div className="flex items-center justify-between">
				...
				{user && isStaff(user.role) && (
					<Button size="sm" onClick={() => setShowCreate(!showCreate)}>
						{showCreate ? <X /> : <Plus />}
						{showCreate ? t("pages.classes.cancelCreate") : t("pages.classes.createClass")}
					</Button>
				)}
			</div>
				...
				<Card>
					<CardContent className="pt-6">
						<form onSubmit={handleCreate} className="flex gap-3">
							<Input placeholder={t("pages.classes.namePlaceholder")} value={name} onChange={e => setName(e.target.value)} required />
							<Input placeholder={t("pages.classes.descriptionPlaceholder")} value={description} onChange={e => setDescription(e.target.value)} />
					...
					<Link key={cls.id} to={`/classes/${cls.id}`}>
						<Card className="transition-shadow hover:shadow-md">
							<CardHeader>
								...
								<CardDescription>{cls.description || t("pages.classes.noDescription")}</CardDescription>
							</CardHeader>
							<CardContent>
								<div className="flex gap-4 text-sm text-muted-foreground">
									<span>{t("pages.classes.memberCount", { count: cls.memberCount })}</span>
									<span>
