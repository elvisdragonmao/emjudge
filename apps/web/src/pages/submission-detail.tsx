...
const ArtifactImage = ({ artifactId, alt, className }: { artifactId: string; alt: string; className?: string }) => {
	...
	if (error) {
		return <div className="flex h-32 items-center justify-center bg-muted text-muted-foreground">{t("pages.submissionDetail.imageLoadFailed")}</div>;
	}

	if (!imageUrl) {
		return <div className="flex h-32 items-center justify-center bg-muted text-muted-foreground">{t("common.loading")}</div>;
	}
...
export const SubmissionDetailPage = () => {
	...
	const { user } = useAuth();
	const { data: submission, isLoading, dataUpdatedAt } = useSubmissionDetail(id!);
	...
	if (!submission) {
		return (
			<>
				...
				<p className="text-muted-foreground">{t("pages.submissionDetail.notFoundTitle")}</p>
			</>
		);
	}

	const canRejudge = !!user && (user.role !== "student" || user.id === submission.userId);
	...
	return (
		<div className="space-y-6">
			...
			<div>
				...
				<h1 className="text-2xl font-bold">{t("pages.submissionDetail.title")}</h1>
			...
			<Card>
				<CardHeader>
					<CardTitle className="text-base">{t("pages.submissionDetail.uploadedFiles")}</CardTitle>
				</CardHeader>
				<CardContent>
								...
								<div className="flex items-center gap-2">
									...
									{canDownloadFiles && (
										<Button size="sm" variant="outline" onClick={() => handleDownloadFile(file.id)} disabled={downloadingFileId === file.id}>
											<Download />
											{downloadingFileId === file.id ? t("pages.submissionDetail.downloading") : t("pages.submissionDetail.download")}
										</Button>
									)}
								</div>
							</div>
				...
				<Card key={run.id}>
					...
					<CardContent className="space-y-4">
						...
						{run.testResults && run.testResults.length > 0 && (
							<div className="space-y-2">
								<h4 className="text-sm font-medium">{t("pages.submissionDetail.tests")}</h4>
								<div className="space-y-1">
						...
						{run.artifacts.length > 0 && (
							<div className="space-y-2">
								<h4 className="text-sm font-medium">{t("pages.submissionDetail.artifacts")}</h4>
								<div className="grid grid-cols-2 gap-2">
									{run.artifacts
										.filter(artifact => artifact.type === "screenshot")
										.map(artifact => (
											<div key={artifact.id} className="overflow-hidden rounded border border-border">
												<ArtifactImage artifactId={artifact.id} alt={artifact.name} className="w-full" />
												<p className="p-1 text-xs text-muted-foreground">{artifact.name}</p>
											</div>
										))}
								</div>
							</div>
