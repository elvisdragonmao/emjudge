			...
			<div className="grid gap-6 md:grid-cols-2">
				...
				<Card>
					...
					<CardContent className="space-y-3">
						<div>
							<span className="text-sm text-muted-foreground">{t("pages.profile.username")}:</span> <span className="text-sm font-medium">{user?.username}</span>
						</div>
						<div>
							<span className="text-sm text-muted-foreground">{t("pages.profile.role")}:</span> <span className="text-sm font-medium">{user?.role ? t(`pages.profile.roles.${user.role}`) : ""}</span>
						</div>
