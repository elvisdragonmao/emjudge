...
import { getSubmissionStatusLabel, getSubmissionStatusVariant } from "@/lib/submission-status";
					...
					<Card className="overflow-hidden transition-shadow hover:shadow-md">
						...
						<div className="aspect-video bg-muted">
							{sub.screenshotUrl ? (
								...
								<div className="flex h-full items-center justify-center text-sm text-muted-foreground">{t("components.submissionGrid.noScreenshot")}</div>
							)}
						</div>
						<CardContent className="p-3">
							...
							<p className="mt-1 text-xs text-muted-foreground">{formatDateTime(sub.createdAt)}</p>
						</CardContent>
					</Card>
				</Link>
			))}
