import { ChangePasswordRequest, MessageResponse, UpdateProfileRequest, UserSummary } from "@judge/shared";
...
export async function meRoutes(app: FastifyInstance) {
	...
	app.post(
		...
		async (request, reply) => {
			...
			const ok = await userService.changePassword(request.userId, body.currentPassword, body.newPassword);
			if (!ok) {
				return reply.status(400).send({ error: "目前密碼不正確", statusCode: 400 });
			}
