...
export const markCompleted = async (
	...
	await query(`UPDATE judge_jobs SET status = 'completed', finished_at = NOW() WHERE id = $1`, [jobId]);
	...
	await query(
     ...
     WHERE id = $5`,
		[results.score, results.maxScore, JSON.stringify(results.testResults), results.log, job.run_id]
	);
...
export const markFailed = async (jobId: string, errorMessage: string) => {
	...
	const finalStatus = job.attempts >= (job.max_attempts || JUDGE_MAX_ATTEMPTS) ? "dead" : "failed";
	const subStatus = finalStatus === "dead" ? "error" : "failed";

	await query(`UPDATE judge_jobs SET status = $1, finished_at = NOW(), error_message = $2 WHERE id = $3`, [finalStatus, errorMessage, jobId]);
	...
	await query(`UPDATE submissions SET status = $1 WHERE id = $2`, [subStatus, job.submission_id]);
